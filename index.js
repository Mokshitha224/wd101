// Function to calculate the date range for age eligibility
function calculateEligibleDateRange() {
  var today = new Date();
  var maxDOB = new Date(today.getFullYear() - 55, today.getMonth(), today.getDate());
  var minDOB = new Date(today.getFullYear() - 18, today.getMonth(), today.getDate());

  return {
      maxDOB: maxDOB.toLocaleDateString(),
      minDOB: minDOB.toLocaleDateString()
  };
}

// Function to display message for eligible date range
function displayEligibleDateRangeMessage() {
  var dateRange = calculateEligibleDateRange();
  var message = "To be eligible (18-55 years old), please select a date of birth between " + dateRange.minDOB + " and " + dateRange.maxDOB + ".";
  alert(message);
}
...
function validateForm() {
  ...
  if (age < 18 || age > 55) {
      displayEligibleDateRangeMessage();
      return false;
  }
